...
**专为安徽理工大学校园网环境打造的智能重连解决方案**

## 🌟 核心特性

### 🔧 智能化配置
- **一次设置，永久生效** - 首次配置后自动保存凭据，无需重复输入
- **多运营商支持** - 自动适配电信、联通、移动三大运营商网络
- **加密存储** - 使用Windows DPAPI安全加密存储用户凭据

### 🚀 自动化运行  
- **开机自启动** - 支持Windows系统开机自动启动
- **后台静默运行** - 最小化到系统托盘，不占用桌面空间
- **智能状态检测** - 每秒自动检测网络连接状态
- **断线自动重连** - 检测到网络断开时立即尝试重新登录

### 💻 用户体验
- **现代化界面** - 基于Qt Widgets的简洁直观界面设计
- **系统托盘集成** - 完整的右键菜单和状态指示功能
- **实时状态显示** - 托盘图标颜色实时反映网络连接状态
- **一键配置修改** - 随时通过托盘菜单修改网络配置
...
#### 运营商选择对应关系
| 运营商类型 | 选择选项 | 服务器标识 | 适用用户 |
|-----------|----------|-----------|----------|
| 中国电信 | 电信 (aust) | `aust` | 电信套餐用户 |
| 中国联通 | 联通 (unicom) | `unicom` | 联通套餐用户 |
| 中国移动 | 移动 (cmcc) | `cmcc` | 移动套餐用户 |
